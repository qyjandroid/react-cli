...
const { CreateCommand } = require('../src/index');

const program = new Command();

program
  .version(package.version, '-v, --version', 'display version for react-cli')
...
program.command('create <name>')
  .description('create a reactTypescriptApp template project')
  .option('-f, --force', '忽略文件夹检查，如果已存在则直接覆盖')
  .action((source, options,command) => {
      new CreateCommand(source, command)
  });
